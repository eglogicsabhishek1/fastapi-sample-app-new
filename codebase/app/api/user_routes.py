...
@router.post(
    ...
    tags=["Users"],
    responses={},  # Add your get_responses if needed
...
async def register_user(
    ...
    if existing_user:
        return JSONResponse(
            ...
            content={"detail": consts.Consts.USER_ALREADY_EXIST}
        )
    # Fetch the role ID for 'user' role
    role = db.query(Role).filter(Role.name == consts.Consts.ROLE_USER).first()
    if not role:
        raise HTTPException(status_code=500, detail="User role not found")

    # Create the user with the fetched role_id
    ...
    created_user = user_crud.create_user(db=db, user=user_to_create, activated=True)
    if not created_user:
        return JSONResponse(
